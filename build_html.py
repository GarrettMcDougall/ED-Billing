...
SECTIONS = [
    ('assess', 'Assessments', '#0ea5e9', [
        ...
        row('General re-assessment (admission assessment, when rendered separately from the initial assessment)', 'C004', '$38.35'),
    ...
    ('crit', 'Critical Care', '#ef4444', [
        ...
        row('First 15 min', 'G521', '$125.10', syn='code blue cardiac arrest crash resuscitation', info="\u201cLife-threatening critical care\u201d — for a patient facing imminent loss of life or organ failure. Time counted is physician time fully devoted to this patient (excludes separately billable interventions); you cannot see other patients during claimed time. Requires start AND stop times documented in the permanent medical record. Not eligible same-day if you're also paid a per-diem ICU/ventilatory/comprehensive/NICU care fee for this patient. Not eligible with A384 or K181."),
        row('2nd 15 min', 'G523', '$64.50', syn='code blue cardiac arrest crash resuscitation', info="\u201cLife-threatening critical care\u201d — for a patient facing imminent loss of life or organ failure. Time counted is physician time fully devoted to this patient (excludes separately billable interventions); you cannot see other patients during claimed time. Requires start AND stop times documented in the permanent medical record. Not eligible same-day if you're also paid a per-diem ICU/ventilatory/comprehensive/NICU care fee for this patient. Not eligible with A384 or K181."),
        row('Every 15 min after', 'G522', '$42.50', syn='code blue cardiac arrest crash resuscitation', info="\u201cLife-threatening critical care\u201d — for a patient facing imminent loss of life or organ failure. Time counted is physician time fully devoted to this patient (excludes separately billable interventions); you cannot see other patients during claimed time. Requires start AND stop times documented in the permanent medical record. Not eligible same-day if you're also paid a per-diem ICU/ventilatory/comprehensive/NICU care fee for this patient. Not eligible with A384 or K181."),
        row('4th doc per 15 min', 'G391', '$34.35', syn='resuscitation code', info="\u201cLife-threatening critical care\u201d — for a patient facing imminent loss of life or organ failure. Time counted is physician time fully devoted to this patient (excludes separately billable interventions); you cannot see other patients during claimed time. Requires start AND stop times documented in the permanent medical record. Not eligible same-day if you're also paid a per-diem ICU/ventilatory/comprehensive/NICU care fee for this patient. Not eligible with A384 or K181."),
        row('Trauma premium (ISS>15)', 'E420', '50%', expand=True),
        ('SUB', 'Other resusc'),
        row('First 15 min', 'G395', '$64.70', syn='resuscitation code', info="\u201cOther critical care\u201d — resuscitation for a threat to life or limb that doesn't meet the higher \u201clife-threatening critical care\u201d bar, where without intervention there's a high probability of limb loss or escalation to life-threatening care. Requires start and stop times documented. Mutually exclusive with the G521/G522/G523 series for the same patient, same day \u2014 bill one series or the other, not both."),
        row('Every 15 min after', 'G391', '$34.35', syn='resuscitation code', info="\u201cOther critical care\u201d — resuscitation for a threat to life or limb that doesn't meet the higher \u201clife-threatening critical care\u201d bar, where without intervention there's a high probability of limb loss or escalation to life-threatening care. Requires start and stop times documented. Mutually exclusive with the G521/G522/G523 series for the same patient, same day \u2014 bill one series or the other, not both."),
        ...
        row('External (transcutaneous) pacing', 'G115', '$46.30', badge='gno', syn='pacer code transcutaneous pacing'),
        ...
        row('Emergency tracheotomy', 'Z325', '$474.65', badge='g35', info="Reserved for a genuine crash airway in a patient who is NOT already intubated \u2014 not for a planned or semi-elective tracheostomy on an already-tubed patient. This includes an emergency cricothyroidotomy. Not for use on an already-intubated patient."),
    ...
    ('lac', 'Lacerations', '#10b981', [
        ...
        row('To SC tissue — one', 'Z080', '$21.90'),
        row('To SC tissue — two', 'Z081', '$32.90'),
        row('To SC tissue — three', 'Z082', '$49.30'),
        row('To SC tissue — four+', 'Z083', '$65.75'),
        row('Tendon/lig/bursa/bone — one', 'Z084', '$65.75'),
        row('Tendon/lig/bursa/bone — 2+', 'Z085', '$98.65'),
        row('Out-of-hospital premium', 'E542', '$12.65'),
    ]),
    ('sedation', 'Procedural Sedation', '#f43f5e', [
        ('NOTE', '⚠ The physician performing the procedure generally cannot also claim the anaesthesia/sedation service for the same patient, same procedure — except for certain nerve blocks. Confirm eligibility before billing both.'),
        ("CALC",),
        ('SUB', 'Reference — raw codes'),
        row('BASE: 6u (procedure code + "C")', '6u', '$95.76'),
        row('Add TIME to base; 15 min = 1u', '1u', '$15.96'),
        row('If no code available (6u base)', 'E003C', '$95.76'),
        row('Exam under anesthesia (6u base)', 'E023C', '$95.76'),
        ('SUB', 'Extra units'),
        row('ASA E — Emergency sedation = 4u', 'E020C', '$63.84'),
        row('ASA III: Severe systemic = 2u', 'E022C', '$31.92'),
        row('ASA IV: Incapacitating = 10u', 'E017C', '$159.60'),
        row('ASA V: Moribund = 20u', 'E016C', '$319.20'),
        row('BMI > 40 = 2u', 'E010C', '$31.92'),
        row('Prone position = 4u', 'E011C', '$63.84'),
        row('Semi-sit (>60°) = 4u', 'E024C', '$63.84'),
        row('Age 29d–1yo = 4u', 'E009C', '$63.84'),
        row('Age 1–8yo = 2u', 'E019C', '$31.92'),
        row('Age 70–79 = 1u', 'E007C', '$15.96'),
        row('Age 80+ = 3u', 'E018C', '$47.88'),
        ('SUB', 'Sedation premiums'),
        row('Weekend/Evening (1700–2400)', 'E400C', '50%'),
        row('Night (0000–0700)', 'E401C', '75%'),
    ]),
    ('misc', 'Misc / HEENT / Blocks', '#6366f1', [
        ...
        row('Ear syringing', 'G420', '$13.15', syn='ear wax cerumen', info='For syringing, curetting, or debridement of the ear canal. Listed among simple office procedures \u2014 confirm it is not bundled into the same-visit assessment fee before billing separately.'),
        row('Epley +', 'G403', '$21.70', syn='vertigo dizzy dizziness bppv'),
        row('Hernia/prolapse reduction', 'Z538', '$25.25'),
        row('ECG (professional component)', 'G313', '$4.55', info='Two components exist: Technical (G310, $7.70) and Professional (G313, $4.55, must include a written interpretation). As a hospital-based ED physician, bill the Professional component. Not payable without symptoms/signs/an indication supported by current clinical practice guidelines.'),
        row('Tetanus by MD', 'G847', '$8.80', syn='tetanus shot immunization'),
        ...
        row('IV start, adult', 'G379', '$6.15', badge='gno', syn='iv line intravenous', info='Bundled into critical care time codes (G395/G391/G521\u2013G523) \u2014 not separately billable alongside those. Billable on its own outside a critical-care encounter.'),
        ...
        row('Foley catheter insertion', 'Z611', '$9.15', badge='gno', syn='urinary catheter bladder catheter', info='Bundled into critical care time codes (G395/G391/G521\u2013G523) \u2014 not separately billable alongside those. Billable on its own outside a critical-care encounter.'),
        ...
        row('Eye X-ray for foreign body (professional component)', 'X016', '$9.90', hidden=True, info='Two components exist: Technical ($14.85) and Professional ($9.90). As a hospital-based ED physician, bill the Professional component.'),
        ...
        ('SUB', 'Ultrasound'),
        row('US-guided aspiration / I&D (professional component)', 'J149', '$56.45', info="Covers real-time ultrasound guidance for a bedside aspiration, drainage, or biopsy you perform yourself \u2014 e.g. an abscess I&D or joint aspiration under direct US visualization. Two components exist: Technical ($47.30, for owning/operating the equipment) and Professional ($56.45, for performing/interpreting). As a hospital-based ED physician using hospital equipment, bill the Professional component only. Don't bill this separately for routine diagnostic or procedural POCUS (see H100)."),
        row('POCUS (emergency department investigative ultrasound)', 'H100', '$19.65', info="Only eligible when: (1) rendered personally by an ED physician meeting training/experience standards; (2) a radiologist is not available for urgent interpretation; and (3) clinically suspecting one of five life-threatening conditions \u2014 pericardial tamponade, cardiac standstill, intraperitoneal haemorrhage (trauma), ruptured AAA, or ruptured ectopic pregnancy. Requires a permanent image record AND an interpretive report. Maximum 2 services per patient per day (2nd only as a follow-up for the same condition). Not for general/undifferentiated POCUS use."),
        ('SUB', 'Nerve blocks lasting > 4h'),
        row('Peripheral nerve, major', 'G060', '$55.00'),
        row('Peripheral nerve, minor', 'G061', '$30.00'),
        row('Local nerve block for procedure', 'G224', '$15.55', info='Unlike most anaesthesia, G224 IS payable to the same physician performing the procedure \u2014 it is the explicit exception to the same-physician anaesthesia rule, for a block lasting more than 4 hours. Limited to once per region per side (bilateral procedures may claim twice).'),
        ...
        row('Inferior alveolar (max/mand)', 'G250', '$75.10', syn='dental block tooth block jaw block', info='Major/minor peripheral nerve block for post-procedure pain control lasting more than 4 hours. Eligible for payment once per region per side (bilateral procedures may claim twice).'),
    ...
    ('fx', 'Fractures', '#f59e0b', [
        ...
        row('Metatarsus, no reduction', 'F061', '$49.20', syn='foot toe forefoot foot ankle hindfoot', fxbadge="no"),
        row('Calcaneus, no reduction', 'F070', '$97.35', syn='heel', fxbadge="no"),
    ...
    ('disloc', 'Dislocations', '#d946ef', [
        ...
        row('AC / SCJ (no reduction)', 'D014', '$67.80', syn='shoulder collarbone acromioclavicular sternoclavicular'),
...
def build():
    ...
    html = (
        ...
        "<meta name=\"theme-color\" content=\"#153d64\">\n"
        "<meta name=\"apple-mobile-web-app-capable\" content=\"yes\">\n"
        "<meta name=\"apple-mobile-web-app-status-bar-style\" content=\"black-translucent\">\n"
        "<meta name=\"apple-mobile-web-app-title\" content=\"ED Billing\">\n"
        + EARLY_THEME_SCRIPT + "\n"
    ...
    # ---- manifest.json ----
    manifest = {
        "name": "ED Billing Guide",
        "short_name": "ED Billing",
